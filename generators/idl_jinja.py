...
parser = argparse.ArgumentParser(description="Generate code from IDL using JINJA templates.")
...
def _pre_process_idl(idl_path):
    ...
    try:
        out = subprocess.Popen(
            ["idlpp", "-l", "pythondesc", idl_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()[0]
    except FileNotFoundError:
        raise RuntimeError(
            "OpenSplice DDS idlpp tool not found. Please install OpenSplice DDS."
        ) from None
