...
parser.add_argument("rdl", type=str, help="RDL File")
parser.add_argument("--templates", type=str, help="Path to templates")
parser.add_argument("--component", type=str, action="append", help="Component Template Name")
...
def main(rdl_file, templates_path, component_templates, out_path):
    ...
    if addrmap_node is not None and is_addrmap(addrmap_node):
    ...
    for d_template in component_templates:
        ...
        component_env = jinja2.Environment(loader=loader, extensions=["jinja2.ext.loopcontrols"])
        ...
        with open(out_file_path, "w") as fout:
            fout.write(
                tmpl.render(root_node=root_node, rld=rdl_file, component_name=component_name)
            )
            fout.close()
