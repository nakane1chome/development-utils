...
parser = argparse.ArgumentParser(description="Generate code from OBJDUMP using JINJA templates.")
parser.add_argument("objdump", type=str, help="OBJDUMP File")
...
SECTION_RE = re.compile(r"Disassembly of section ([\w\.]+)\:")
SYMBOL_RE = re.compile(r"([0-9a-fA-F]+)\s+\<(.+?)\>:")
ADDR_RE = re.compile("([0-9a-fA-F]+):")
FILE_RE = re.compile(r"(.*?):\s+file format (.*)")
LINE_NUMBERS_RE = re.compile(r"(.*?)\:(\d+)$")
...
def parse_objectdump(filename):
    ...
    with open(filename) as fin:
        ...
        for line in fin.readlines():
            # m0 = SECTION_RE.match(line)  # unused
            ...
            if m3 is not None:
                ...
                current_line = (
                    ...
                    line_no,
                    f"{os.path.basename(file_name)}:{line_no}",
                )
            elif m1 is not None:
                ...
                parts = re.compile(r"\s+").split(data)
            ...
            if addr is not None and section is not None and by_section[section]["addr"] is None:
